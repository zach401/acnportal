...
def evse_violation(env):
    """ If a single EVSE constraint is violated, a negative reward 
    equal to the magnitude of the violation is added to the total 
    reward
    """
    violation = 0
    for i in range(len(env.interface.evse_list)):
        ...
        if action[i] < curr_evse.min_rate:
            violation -= curr_evse.min_rate - action[i]
        if action[i] > curr_evse.max_rate:
            violation -= action[i] - curr_evse.max_rate
    return violation

def unplugged_ev_violation(env):
    """ If charge is attempted to be delivered to an evse with 
    no ev, this rate is subtracted from the reward.
    """
    violation = 0
    for i in range(len(env.interface.evse_list)):
        if env.interface.evse_list[i].ev is None:
            violation -= abs(action[i])
    return violation

def constraint_violation(env):
    """ If a network constraint is violated, a negative reward equal 
    to the abs of the constraint violation, times the number of 
    EVSEs, is added.
    """
    _, magnitudes = env.interface.network_constraints
    # Calculate aggregate currents for this charging schedule
    outvec = abs(env.interface.constraint_currents(
    ...
    # EVSEs
    violation = \
        np.linalg.norm(diffvec) * env.num_evses * (-1)
    return violation
...
def soft_charging_reward(env):
    ...
    charging_reward = np.sum(
        self.interface._simulator.charging_rates[:, 
