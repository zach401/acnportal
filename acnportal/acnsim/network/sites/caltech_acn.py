...
def caltech_acn(basic_evse=False, voltage=208, transformer_cap=150, network_type=ChargingNetwork):
    ...
    for evse_id in AB_ids:
        if evse_id not in CC_pod_ids:
            network.register_evse(get_evse_by_type(evse_id, evse_type['AV']), voltage, 30)
        else:
            network.register_evse(get_evse_by_type(evse_id, evse_type['CC']), voltage, 30)
...
def CaltechACN(basic_evse=False, voltage=208, transformer_cap=150, network_type=ChargingNetwork):
    """ Wrapper around caltech_acn for backward compatibility. Will be removed in future release. """
    print('WARNING: CaltechACN will be removed in a future release. Please use caltech_acn().')
